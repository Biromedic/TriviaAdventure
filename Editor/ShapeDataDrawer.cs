...
public class ShapeDataDrawer : Editor
{
    ...
    private void DrawColumnsInputFields()
    {
        ...
        int newColumns = EditorGUILayout.IntField("Columns", ShapeDataInstance.columns);
        int newRows = EditorGUILayout.IntField("Rows", ShapeDataInstance.rows);

        if (newColumns != columnsTemp || newRows != rowsTemp)
        {
            if (newColumns > 0 && newRows > 0)
            {
                ShapeDataInstance.columns = newColumns;
                ShapeDataInstance.rows = newRows;
    ...
    private void DrawBoardTable()
    {
        ...
        for (int row = 0; row < ShapeDataInstance.rows; row++)
        {
            ...
            for (int column = 0; column < ShapeDataInstance.columns; column++)
            {
                ...
                if (row < ShapeDataInstance.board.Length && column < ShapeDataInstance.board[row].column.Length)
                {
                ...
                else
                {
                    ...
                    EditorGUILayout.Toggle(false, dataFieldStyle);
                    EditorGUI.EndDisabledGroup();
                    EditorGUILayout.LabelField("Out of bounds", GUILayout.Width(80));
                }
